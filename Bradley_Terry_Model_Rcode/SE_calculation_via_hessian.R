...
solver <- function(abilities,likelihood,gradient,lambda,compute_hessian = TRUE){
...
total_SE <- 0
total_ab <- 0
SE_matrix <- matrix(NA_real_, nrow = length(abilities), ncol = k_folds)
...
for (j in 1:k_folds){
  ...
  lambda <- regularisation_assigner(results,lambda_val_1 = 2.7,lambda_val_2 = 0.96)
  optim_results <- solver(abilities,neg_log_likelihood,gradient,lambda)
  player_scores <- optim_results$player_scores
  
  total_SE <- total_SE + optim_results$SE_vector
  total_ab <- total_ab + player_scores
  SE_matrix[,j] <- optim_results$SE_vector
  
}

SE_vector_avg <- total_SE/k_folds #standard error of estimated player abilities average
ab_vector_avg <- total_ab/k_folds #estimated player abilities average


#SE_vector_med <- apply(SE_matrix, 1, median)   # one median per row
